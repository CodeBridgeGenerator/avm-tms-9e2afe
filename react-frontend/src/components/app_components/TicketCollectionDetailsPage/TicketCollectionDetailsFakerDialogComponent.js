...
import ticketCollectionDetailsFakerFactory from "./ticketCollectionDetailsFakerFactory";
...
const TicketCollectionDetailsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [userNameIds, setuserNameIds] = useState();
const [userMailAddressIds, setuserMailAddressIds] = useState();
...
client.service("users")
                        ...
                        .then((data) => {
                        setuserMailAddressIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = ticketCollectionDetailsFakerFactory(props.user,fakerCount,userNameIds,userMailAddressIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("ticketCollectionDetails").create(elem));
...
export default connect(mapState, mapDispatch)(TicketCollectionDetailsFakerDialogComponent);
