...
import TicketCollectionDetailsDatatable from "./TicketCollectionDetailsDataTable";
import TicketCollectionDetailsEditDialogComponent from "./TicketCollectionDetailsEditDialogComponent";
import TicketCollectionDetailsCreateDialogComponent from "./TicketCollectionDetailsCreateDialogComponent";
import TicketCollectionDetailsFakerDialogComponent from "./TicketCollectionDetailsFakerDialogComponent";
import TicketCollectionDetailsSeederDialogComponent from "./TicketCollectionDetailsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const TicketCollectionDetailsPage = (props) => {
    ...
    const filename = "ticketCollectionDetails.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("ticketCollectionDetails");
    ...
    useEffect(() => {
        ...
        props.show();
        client
            .service("ticketCollectionDetails")
            .find({ query: { $limit: 10000 , userName : urlParams.singleUsersId ,userMailAddress : urlParams.singleUsersId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "TicketCollectionDetails", type: "error", message: error.message || "Failed get TicketCollectionDetails" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("ticketCollectionDetails").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "TicketCollectionDetails", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("ticketCollectionDetails").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "TicketCollectionDetails",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/ticketCollectionDetails/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>TicketCollectionDetails </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="ticketCollectionDetails-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="ticketCollectionDetails-datatable">
                    <TicketCollectionDetailsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <TicketCollectionDetailsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <TicketCollectionDetailsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <TicketCollectionDetailsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <TicketCollectionDetailsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(TicketCollectionDetailsPage);
