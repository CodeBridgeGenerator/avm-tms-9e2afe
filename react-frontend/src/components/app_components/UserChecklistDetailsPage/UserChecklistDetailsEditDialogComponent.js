...
const UserChecklistDetailsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            checkListName: _entity?.checkListName,
checkListDescription: _entity?.checkListDescription,
checkListItems: _entity?.checkListItems,
checkListOrder: _entity?.checkListOrder,
        };

        setLoading(true);
        try {
            
        const result = await client.service("userChecklistDetails").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info userChecklistDetails updated successfully" });
    ...
    return (
        <Dialog header="Edit UserChecklistDetails" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="userChecklistDetails-edit-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="checkListItems" className="w-full mb-3 p-inputtext-sm" value={_entity?.checkListItems} onChange={(e) => setValByKey("checkListItems", e.target.value)}  />
            </span>
            <small className="p-error">
